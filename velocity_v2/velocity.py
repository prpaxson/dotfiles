...
def getDirectoryText():
    username = pwd.getpwuid(os.getuid())[0]
    return os.getcwd().replace("/Users/"+username, "~", 1).replace("/home/"+username, "~", 1)
...
def promptMain():
    ...
    if theme == 'light':
        ...
        gitCleanFormat    = Format('magenta', 'black')
        gitDirtyFormat    = Format('green', 'black')
        gitDetachedFormat = Format('red', 'black')
...
def tmuxStatusRightMain():
    ...
    if getTmuxOption("@STATUSRIGHTAUTOSCALE", "g", "false") == "true":
        ...
        baseCutoff = int(getTmuxOption("@AUTOSCALECUTOFF", "g", "200"))
